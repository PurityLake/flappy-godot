...
func _process(delta):
	...
	self.velocity.y = clamp(self.velocity.y, -7, 7)
	...
	if self.rotation > 0 and dir < 0:
		self.rotation = clampf(self.rotation + dir * fast_move * delta, -max_angle, max_angle)
	elif self.rotation < 0 and dir > 0:
		self.rotation = clampf(self.rotation + dir * fast_move * delta, -max_angle, max_angle)
	else:
		self.rotation = clampf(self.rotation + dir * angle_move * delta, -max_angle, max_angle)
